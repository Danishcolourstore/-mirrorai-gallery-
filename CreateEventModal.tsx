...
import { Grid2X2, LayoutGrid, AlignJustify, Newspaper, GalleryHorizontalEnd, Clapperboard, Sparkles, LayoutDashboard, Instagram } from 'lucide-react';
...
const GALLERY_STYLES = [
  {
    ...
    label: 'Standard',
    icon: '📷',
    desc: 'Direct photo grid. Clean & simple.',
  },
  {
    value: 'studio',
    label: 'Studio',
    icon: '✦',
    desc: 'Branded landing page → then gallery. Premium feel.',
  },
] as const;
...
export function CreateEventModal({ open, onOpenChange, onCreated }: CreateEventModalProps) {
  ...
  const [galleryStyle, setGalleryStyle] = useState<'standard' | 'studio'>('standard');
  const [galleryMode, setGalleryMode] = useState('mirrorai');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Auto-generate FTP password for MirrorLive
    const ftpChars = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789';
    const ftpPassword = Array.from(crypto.getRandomValues(new Uint8Array(12)))
      .map(b => ftpChars[b % ftpChars.length])
      .join('');
    ...
    if (error) {
      ...
      setGalleryLayout('classic'); setDownloadsEnabled(true); setGalleryStyle('standard'); setGalleryMode('mirrorai');
      onOpenChange(false);
      ...
      <DialogContent className="sm:max-w-[420px] bg-card border-border p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5 mt-1">
          ...
          <div className="space-y-2">
            ...
            <GalleryModeSelector value={galleryMode} onChange={setGalleryMode} userId={user?.id} />
          </div>

          {/* Gallery Style Selector */}
          <div className="space-y-2">
            <Label className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground/70 font-medium">Choose your gallery style</Label>
            <div className="grid grid-cols-2 gap-2">
              {GALLERY_STYLES.map((style) => (
                <button
                  key={style.value}
                  type="button"
                  onClick={() => setGalleryStyle(style.value)}
                  className={`flex flex-col items-center gap-1.5 py-4 px-3 border transition-all text-center rounded-sm ${
                ...
                >
                  ...
                  <span className="text-[11px] font-medium tracking-wide">{style.label}</span>
                  <span className="text-[9px] text-muted-foreground/50 leading-snug">{style.desc}</span>
                </button>
              ))}
            </div>
          </div>
